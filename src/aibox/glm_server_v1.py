...
CPU-SAFE PATCH: Fixed apply_rotary_pos_emb dimension mismatch on CPU
"""
import asyncio
import base64
import datetime
...
# >>>>> CPU 优化：强制使用 float32，禁用 bfloat16 <<<<<
...
if DEVICE == "cpu":
    ...
    log.info("[cpu] optimized: threads=4, mkldnn=%s, dtype=float32",
...
# >>>>> 新增：CPU 安全的 RoPE 补丁函数 <<<<<
def _patch_glm_rotary_emb(model) -> None:
    """
    Patch GLM model's apply_rotary_pos_emb to be CPU-safe.
    Fixes: RuntimeError: Sizes of tensors must match except in dimension 3
    """
    import torch
    ...
    def safe_apply_rotary_pos_emb(q, k, cos, sin, position_ids=None, unsqueeze_dim=1, partial_rotary_factor=0.5):
        """CPU-safe version of apply_rotary_pos_emb"""
        # Ensure all tensors on same device and dtype
        device = q.device
        cos = cos.to(device=device, dtype=q.dtype)
        sin = sin.to(device=device, dtype=q.dtype)

        # Handle cos/sin dimension expansion robustly
        if cos.dim() == 2:  # [seq_len, head_dim//2]
            cos = cos.unsqueeze(0).unsqueeze(unsqueeze_dim)  # [1, 1, seq_len, head_dim//2]
            sin = sin.unsqueeze(0).unsqueeze(unsqueeze_dim)
        elif cos.dim() == 3:  # [batch, seq_len, head_dim//2]
            cos = cos.unsqueeze(unsqueeze_dim)  # [batch, 1, seq_len, head_dim//2]
        ...
        # Slice cos/sin to match rotary dim
        cos_rotary = cos[..., :rotary_dim]
        sin_rotary = sin[..., :rotary_dim]

        # Apply RoPE
        q_rotary = (q_rotary * cos_rotary) + (rotate_half(q_rotary) * sin_rotary)
        k_rotary = (k_rotary * cos_rotary) + (rotate_half(k_rotary) * sin_rotary)

        # Concatenate with shape safety
        q_embed = torch.cat([q_rotary, q_pass], dim=-1)
        k_embed = torch.cat([k_rotary, k_pass], dim=-1)

        return q_embed, k_embed

    # Patch at module level if possible
    try:
        module_name = model.model.__class__.__module__
        if module_name in sys.modules:
            mod = sys.modules[module_name]
            if hasattr(mod, 'apply_rotary_pos_emb'):
                original_func = mod.apply_rotary_pos_emb
                mod.apply_rotary_pos_emb = safe_apply_rotary_pos_emb
                log.info("[patch] replaced apply_rotary_pos_emb in module %s", module_name)
    except Exception as e:
        log.warning("[patch] module-level patch failed: %s", e)

    # Store as fallback
    model._safe_rotary_emb = safe_apply_rotary_pos_emb
...
def _load_model(name: str) -> dict:
    ...
    load_kwargs = dict(
        ...
        attn_implementation="eager",  # 关键：禁用 SDPA/FlashAttention
    )
    ...
    # >>>>> 应用 CPU RoPE 补丁 <<<<<
    if DEVICE == "cpu":
        _patch_glm_rotary_emb(model)
        log.info("[patch] applied CPU-safe RoPE patch for %s", name)
...
def _build_gen_kwargs(bundle: dict, req: ChatCompletionRequest) -> tuple[dict, int, bool]:
    ...
    if is_vision:
        if images:
            ...
            pixel_values = image_processor(merged, return_tensors="pt").pixel_values
            gen_kwargs["pixel_values"] = pixel_values.to(device=DEVICE, dtype=DTYPE)
        else:
