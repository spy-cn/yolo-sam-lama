...
# YOLO_EAR_PATH = str(BASE_DIR / "models")
# settings.update({"weights_dir": YOLO_EAR_PATH})

# 验证是否修改成功
# print("当前全局权重存放路径为：", settings["weights_dir"])
YOLO26L = str(BASE_DIR / "models/yoloe-26l-seg.pt")

from ultralytics import YOLO

# Initialize model
# model = YOLO(YOLO26L)  # or select yoloe-26s/m-seg.pt for different sizes
#
# # Set text prompt to detect person and bus. You only need to do this once after you load the model.
# model.set_classes(["person", "bus"])
#
# # Run detection on the given image
# results = model.predict(r"C:\pablo\05_self_code\yolo-sam-lama\data\person\person_road_03.jpeg")
#
# # Show results
# results[0].show()


def auto_ensure_yolo26_person(
        image_path: str,
        output_path: str,
        target_classes: list,
